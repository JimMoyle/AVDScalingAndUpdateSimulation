function Invoke-Batch {
    ...
    PROCESS {
        ...
        while ($hp.Servers.Version -contains 1) {
            ...
            $hp.Servers | Sort-Object -Property CurrentUserCount | Select-Object -First $serversToDrain | ForEach-Object {
                $_.AllowNewConnections = $false
            }

            $drainingServers = $hp.Servers | Where-Object {$_.AllowNewConnections -eq $false}

            if (($drainingServers.CurrentUserCount | Measure-Object -Sum) -eq 0) {
                $hp.Servers | Where-Object { $_.AllowNewConnections = $false } | ForEach-Object {
                    $_.version++
                    $_.AllowNewConnections = $true
                }
            }
